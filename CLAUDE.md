...
node --test Scripts/test-block-serializer.js   # block parse/serialize round-trip corpus only
...
Sources/QuillKit/
  ...
  Resources/        editor.html (Tiptap)
                    ...
                    block-parser-bundle.js (window.BlockParser — WordPress's own block parser)
                    block-serializer.js (block tree → post_content), block-descriptors.js
  DesignSystem.swift
```
