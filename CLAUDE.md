...
## Test suite status (2026-06-17 — 259 Swift + 98 JS tests, all passing)

**Swift (259 tests):** 18 suites covering all models, WordPressClient, all storage layers, AIPromptBuilder, AnthropicClient, AppState view-model logic, EditorCoordinator, status helpers, and PostStats. Each network suite uses its own MockURLProtocol subclass to avoid global-state races.

**JS (92 tests):** `Scripts/test-editor.js` covers `toWordPressHTML`, `extractAlignment`, `formatHTML`, `countStats`, `findMatches`, `detectEmbedProvider`, and `embedClassFor` via Node + jsdom (headings, lists, blockquotes, code blocks, images, tables, embeds, footnotes, footnote backrefs, idempotency, unicode).
...
- **`RenderedString.editorHTML` computed property** — use this instead of `raw ?? rendered` anywhere you load post/page content or excerpt into the editor. `raw` can be an empty string `""` or whitespace-only `"\n  "` (WordPress sends this for posts with no `context=edit` access), which would clear the editor even though `rendered` has valid HTML. `editorHTML` returns `raw` only when it is non-empty after trimming; otherwise falls back to `rendered`. Do NOT revert call sites to `raw ?? rendered` or `raw ?? ""`.
