...
- **Spelling suggestions in the context menu** — `rightMouseDown` evaluates `window.spellContextAtPoint?(x, y)` which returns `{ word, from, to }` (ProseMirror character positions) when the click lands on a spell-error decoration, or `null` otherwise. `SpellContext` (private struct) and `SpellReplacement: NSObject` (private class — must be `NSObject` for use as `representedObject`) carry the data. `spellingSuggestions(for:)` uses `NSSpellChecker.guesses`; top 8 results are prepended above Cut/Copy/Paste with a separator. Applying a suggestion calls `window.replaceSpellError?.(from, to, jsonEncodedSuggestion)` which dispatches `state.tr.insertText` in ProseMirror. The `applySpellingSuggestion(_:)` selector must be in `WebViewMenuFilter.allowed`.
