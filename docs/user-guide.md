...
**Unsupported Gutenberg blocks.** If your post contains a block Quill doesn't natively support (Accordion, Columns, Group, or a third-party plugin block), it appears in the visual editor as a card labeled with the block's name and the hint "Unsupported block — edit via Code View." This card preserves the block's original markup exactly, including any nested content, and survives edits you make elsewhere in the post — it's only lost if you delete the card itself. To edit the block's own content, use code view.
