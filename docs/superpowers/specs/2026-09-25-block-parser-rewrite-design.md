...
- **Comparison with WordPress.** For each input, `parseBlocks` must deep-equal the reference parser's output once `start`, `end` and `closed` are removed. Inputs:
  ...
  - 2,000 inputs from a seeded generator built from openers, closers and self-closing comments with varied names, valid and invalid attributes, attributes containing `}` and `-->`, near-miss comments, text and whitespace. An input is skipped when `parseBlocks` marks two or more blocks `closed: false`, and the test fails if more than a quarter of inputs are skipped, so the exception cannot quietly swallow the corpus. (The generator cannot track depth itself: an attribute variant such as `{"s":"}  -->"}` turns a self-closing comment into an opening one.)
...
- `docs/wordpress-release-audit.md`: a step that moves the pinned reference parser in `Scripts/package.json` to its latest version and runs `Scripts/test-block-parser.js`. A failure there means WordPress changed how it reads block comments, and the failing input shows where. The parser's output has not changed since 2018, so this is expected to pass; its job is to make a change visible if one ever comes.
